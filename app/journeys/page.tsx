...
import JourneyModal, { JourneyData } from '@/components/JourneyModal';

// Reference-inspired immersive loader - child's hand-drawn dream in deep space
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#0A162F] flex items-center justify-center overflow-hidden">
      <div className="site-loader__content w-full max-w-md px-6 text-[#94E6FB]">
        ...
        {/* Progress circle + labels matching reference */}
...
// Three hand-drawn space dream journeys for BYDT
...
export default function JourneysPage() {
  ...
  const [modalJourney, setModalJourney] = useState<JourneyData | null>(null);
  ...
  const handleSelectJourney = (journey: JourneyData) => {
    ...
    // Update HUD and canvas immediately (like selecting a "current")
  ...
  return (
    <>
      {/* Custom immersive header for the experience - minimal like the reference */}
      ...
      {/* Immersive Starfield Background - child's hand-drawn deep space dream (full-bleed like the reference) */}
      <div className="fixed inset-0 z-0">
        <StarfieldCanvas activeJourney={activeJourneyId || undefined} />
      </div>

      {/* Subtle hand-drawn decorative frame elements for the "sketchbook in space" feel */}
      ...
      {/* Reference-style HUD - updated live with journey selection, with SVG viewer */}
      <Hud
      ...
      {/* Audio button - like the reference (visual toggle; real audio can be wired with actual files) */}
      <div className="fixed top-20 left-6 z-50">
        <button
          onClick={() => { /* toggle audio logic here if files added */ }}
        ...
        >
          ...
          <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M10 3.5v13l-4-3H3a1 1 0 01-1-1V7.5a1 1 0 011-1h3l4-3z" /></svg>
        </button>
      </div>

      <main className="relative z-10 min-h-screen pt-20 pb-24 text-cream">
        {/* Title area - dreamy, child's perspective */}
        ...
        {/* Three Journeys as prominent "currents" - hand-drawn dream style */}
        <div className="max-w-5xl mx-auto px-6 mt-12 grid md:grid-cols-3 gap-6">
          {JOURNEYS.map((journey) => {
            ...
            return (
              <button
                ...
                className={`group text-left rounded-2xl overflow-hidden border border-gold/20 bg-navy/60 backdrop-blur-sm transition-all duration-300 hover:border-gold/60 hover:-translate-y-1 ${isActive ? 'ring-2 ring-gold border-gold' : ''}`}
              >
                ...
                <div className="p-7">
                  <div className="uppercase tracking-[3px] text-xs text-gold font-mono mb-2">JOURNEY CURRENT</div>
                  <h3 className="text-2xl leading-tight mb-3 group-hover:text-gold transition-colors">{journey.title}</h3>
                  <p className="text-[#C5BBAE] text-sm mb-5 line-clamp-3">{journey.spotlight}</p>
                  <div className="btn btn-primary text-sm inline-flex">
                    Float In
                  </div>
                </div>
              </button>
        ...
        {/* Central breathing + hand-drawn encouragement */}
        <div className="max-w-xl mx-auto px-6 mt-16 text-center">
          ...
          <p className="text-[#C5BBAE]">
            Every journey begins when a child notices a spark in the dark.<br />
            Here, that spark becomes a constellation you can actually hold.
          </p>
        </div>
      </main>
      ...
      {/* Rich Journey Modal - child's hand-drawn spotlight, matching reference depth */}
      <JourneyModal
        isOpen={isModalOpen}
        onClose={closeModal}
...
// Re-export a local copy of BreathingStar for this page (lightweight)
